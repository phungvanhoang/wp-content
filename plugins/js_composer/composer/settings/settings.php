...
class WPBakeryVisualComposerSettings {
    ...
    public function __construct($composer) {
        ...
        if(!$composer->updaterDisabled()) $this->tabs['updater'] = __('Product License', 'js_composer');
...
* @param $callback - function to build section header.
    ...
    public function column_css_classes_callback() {
        ...
        ?>
    <?php // <p class="description indicator-hint"> _e("To change class names for the columns elements, enter them here. By default vc_spanX are used, where X number from 1 to 12.", "js_composer");</p> ?>
    <?php
    }
    /**
     * Not responsive checkbox callback function
    ...
    public function setting_section_callback_function($tab) {
       ...
       <div class="tab_intro">
           <p>
               ...
               <?php _e('A valid license key qualifies you for support and enables automatic updates. A license key may only be used for one Visual Composer installation on one WordPress site at a time.', 'js_composer') ?>
           </p>
       </div>
        <?php endif;
    }
...
EOF;
            ...
            <a href="#" class="button vc-restore-button" id="vc-settings-custom-css-reset-data"><?php _e('Remove all saved', "js_composer") ?></a>
        </form>
